...
public class UserAdditionService implements UserService {
    ...
    public GetUserDto addUser(AddUserDto addUserDto) {
        ...
        return userToGetUserDtoMapper.map(savedUser);
    }
    ...
    private void addToFriend(Long userId, Long friendId){
        ...
        if(user.getFollowedUsers().stream().anyMatch(friend -> friend.getId().equals(friendId))){
            throw new ThisUserAlreadyExistsOnUserFriendsListException();
        }
        User friend = userGetService.getUserById(friendId);
        List<User> friends = user.getFollowedUsers();
        friends.add(friend);
        user.setFollowedUsers(friends);
    }
}
